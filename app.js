...
function initDeviceListeners() {
    devices.forEach(device => {
        ...
        deviceRef.on('value', (snapshot) => {
            ...
            if (card) {
                ...
                if (state === 1) {
                    ...
                    card.style.boxShadow = '0 0 15px rgba(255, 250, 205, 0.5)';
                } else {
        ...
        // Also listen for toggle changes
        document.querySelectorAll('.device-card').forEach(card => {
            const toggle = card.querySelector('.device-toggle');
            toggle.addEventListener('change', (e) => {
                ...
                if (isOn) {
                    card.classList.add('active');
                    showNotification(`${card.querySelector('h3').textContent} turned on`, 'success');
                    ...
                    card.classList.remove('active');
                    showNotification(`${card.querySelector('h3').textContent} turned off`, 'warning');
                }
            });
        });
    });
...
function updateDeviceState(deviceId, state) {
    database.ref(`led/${deviceId}`).set(state ? 1 : 0)
        .then(() => console.log(`${deviceId} state updated`))
        .catch(error => {
